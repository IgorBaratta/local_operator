...
do
  ...
  grep -e "L2 data volume" output/L2* >> likwid/laplacian_L2_${BATCH_SIZE}_${DEGREE}_${OPTIMIZE}_${PRECOMPUTE}_512.txt
  grep -e "L2 bandwidth" output/L2* >> likwid/laplacian_L2_${BATCH_SIZE}_${DEGREE}_${OPTIMIZE}_${PRECOMPUTE}_512.txt
  
  mpirun -n ${NUM_PROCS} likwid-perfctr -m -C E:N:1 -g MEM_DP -o output/MEM_DP_%h_%r.txt ./build/benchmark
  grep -e "Memory data volume" output/MEM_DP* >> likwid/laplacian_MEM_DP_${BATCH_SIZE}_${DEGREE}_${OPTIMIZE}_${PRECOMPUTE}_512.txt
  grep -e "Memory bandwidth" output/MEM_DP* >> likwid/laplacian_MEM_DP_${BATCH_SIZE}_${DEGREE}_${OPTIMIZE}_${PRECOMPUTE}_512.txt
  
  mpirun -n ${NUM_PROCS} likwid-perfctr -m -C E:N:1 -g FLOPS_DP -o output/FLOPS_DP_%h_%r.txt ./build/benchmark
  grep -e "FP_DP_FIXED_OPS_SPEC" output/FLOPS_DP* >> likwid/laplacian_FLOPS_DP_${BATCH_SIZE}_${DEGREE}_${OPTIMIZE}_${PRECOMPUTE}_512.txt
  grep -e "FP_DP_SCALE_OPS_SPEC" output/FLOPS_DP* >> likwid/laplacian_FLOPS_DP_${BATCH_SIZE}_${DEGREE}_${OPTIMIZE}_${PRECOMPUTE}_512.txt
done
