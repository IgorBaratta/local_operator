...
struct Operator
{
    constexpr static std::size_t num_dofs = 3 * (P + 1) * (P + 1) * (P + 1);
    inline void apply(T *restrict A, const T *restrict w, const T *restrict jacobian)
    {
            ...
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < Nd; ++ic0)
                    for (int id = 0; id < Nd * Nd; ++id)
                        temp0[Nd * Nd * iq0 + id] += FE_TF0[iq0][ic0] * w[Nd * Nd * ic0 + id + 8];
            T temp1[Nd * Nq * Nq] = {0};
            T temp0transp[Nd * Nd * Nq] = {0};
        ...
        }
        {
            ...
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < Nd; ++ic0)
                    for (int id = 0; id < Nd * Nd; ++id)
                        temp0[Nd * Nd * iq0 + id] += FE_TF1[iq0][ic0] * w[Nd * Nd * ic0 + id + 8];
            T temp1[Nd * Nq * Nq] = {0};
            T temp0transp[Nd * Nd * Nq] = {0};
            for (int ic1 = 0; ic1 < Nd; ++ic1)
                for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int id = 0; id < Nq * Nq; ++id)
                        w1_d[Nq * Nq * iq2 + id + 3 * cubNq] += FE_TF1[iq2][ic2] * temp1transp[Nq * Nq * ic2 + id];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int id = 0; id < Nq * Nq; ++id)
                        w1_d[Nq * Nq * iq2 + id + 6 * cubNq] += FE_TF0[iq2][ic2] * temp1transp[Nq * Nq * ic2 + id];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < Nd; ++ic0)
                    for (int id = 0; id < Nd * Nd; ++id)
                        temp0[Nd * Nd * iq0 + id] += FE_TF0[iq0][ic0] * w[Nd * Nd * ic0 + id + 1 * cubNd + 8];
            T temp1[Nd * Nq * Nq] = {0};
            T temp0transp[Nd * Nd * Nq] = {0};
            for (int ic1 = 0; ic1 < Nd; ++ic1)
                for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int id = 0; id < Nq * Nq; ++id)
                        w1_d[Nq * Nq * iq2 + id + cubNq] += FE_TF1[iq2][ic2] * temp1transp[Nq * Nq * ic2 + id];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < Nd; ++ic0)
                    for (int id = 0; id < Nd * Nd; ++id)
                        temp0[Nd * Nd * iq0 + id] += FE_TF1[iq0][ic0] * w[Nd * Nd * ic0 + id + 1 * cubNd + 8];
            T temp1[Nd * Nq * Nq] = {0};
            T temp0transp[Nd * Nd * Nq] = {0};
            for (int ic1 = 0; ic1 < Nd; ++ic1)
                for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int id = 0; id < Nq * Nq; ++id)
                        w1_d[Nq * Nq * iq2 + id + 4 * cubNq] += FE_TF1[iq2][ic2] * temp1transp[Nq * Nq * ic2 + id];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int id = 0; id < Nq * Nq; ++id)
                        w1_d[Nq * Nq * iq2 + id + 7 * cubNq] += FE_TF0[iq2][ic2] * temp1transp[Nq * Nq * ic2 + id];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < Nd; ++ic0)
                    for (int id = 0; id < Nd * Nd; ++id)
                        temp0[Nd * Nd * iq0 + id] += FE_TF0[iq0][ic0] * w[Nd * Nd * ic0 + id + 2 * cubNd + 8];
            T temp1[Nd * Nq * Nq] = {0};
            T temp0transp[Nd * Nd * Nq] = {0};
            for (int ic1 = 0; ic1 < Nd; ++ic1)
                for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int id = 0; id < Nq * Nq; ++id)
                        w1_d[Nq * Nq * iq2 + id + 2 * cubNq] += FE_TF1[iq2][ic2] * temp1transp[Nq * Nq * ic2 + id];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < Nd; ++ic0)
                    for (int id = 0; id < Nd * Nd; ++id)
                        temp0[Nd * Nd * iq0 + id] += FE_TF1[iq0][ic0] * w[Nd * Nd * ic0 + id + 2 * cubNd + 8];
            T temp1[Nd * Nq * Nq] = {0};
            T temp0transp[Nd * Nd * Nq] = {0};
            for (int ic1 = 0; ic1 < Nd; ++ic1)
                for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int id = 0; id < Nq * Nq; ++id)
                        w1_d[Nq * Nq * iq2 + id + 5 * cubNq] += FE_TF1[iq2][ic2] * temp1transp[Nq * Nq * ic2 + id];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int id = 0; id < Nq * Nq; ++id)
                        w1_d[Nq * Nq * iq2 + id + 8 * cubNq] += FE_TF0[iq2][ic2] * temp1transp[Nq * Nq * ic2 + id];
        }
        {
            T temp0[4 * Nq] = {0};
            T wtransp[Nq] = {0};
            for (int ic0 = 0; ic0 < 2; ++ic0)
                for (int ic1 = 0; ic1 < 2; ++ic1)
                    for (int ic2 = 0; ic2 < 2; ++ic2)
                        wtransp[4 * ic0 + 2 * ic1 + ic2] += w[4 * ic0 + 2 * ic1 + ic2];
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < 2; ++ic0)
                    for (int id = 0; id < 4; ++id)
                        temp0[4 * iq0 + id] += FE_TF2[iq0][ic0] * wtransp[4 * ic0 + id];
            T temp1[2 * Nq * Nq] = {0};
...
#else
        {
            ...
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < Nd; ++ic0)
                    for (int ic1 = 0; ic1 < Nd; ++ic1)
                        for (int ic2 = 0; ic2 < Nd; ++ic2)
                            temp0[Nd * Nd * iq0 + Nd * ic1 + ic2] += FE_TF0[iq0][ic0] * w[Nd * Nd * ic0 + Nd * ic1 + ic2 + 8];
            T temp1[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int ic1 = 0; ic1 < Nd; ++ic1)
                    for (int iq0 = 0; iq0 < Nq; ++iq0)
        ...
        }
        {
            ...
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < Nd; ++ic0)
                    for (int ic1 = 0; ic1 < Nd; ++ic1)
                        for (int ic2 = 0; ic2 < Nd; ++ic2)
                            temp0[Nd * Nd * iq0 + Nd * ic1 + ic2] += FE_TF1[iq0][ic0] * w[Nd * Nd * ic0 + Nd * ic1 + ic2 + 8];
            T temp1[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int ic1 = 0; ic1 < Nd; ++ic1)
                    for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq0 = 0; iq0 < Nq; ++iq0)
                            w1_d[Nq * Nq * iq0 + Nq * iq1 + iq2 + 3 * cubNq] += FE_TF1[iq2][ic2] * temp1[Nd * Nq * iq1 + Nd * iq0 + ic2];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq0 = 0; iq0 < Nq; ++iq0)
                            w1_d[Nq * Nq * iq0 + Nq * iq1 + iq2 + 6 * cubNq] += FE_TF0[iq2][ic2] * temp1[Nd * Nq * iq1 + Nd * iq0 + ic2];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < Nd; ++ic0)
                    for (int ic1 = 0; ic1 < Nd; ++ic1)
                        for (int ic2 = 0; ic2 < Nd; ++ic2)
                            temp0[Nd * Nd * iq0 + Nd * ic1 + ic2] += FE_TF0[iq0][ic0] * w[Nd * Nd * ic0 + Nd * ic1 + ic2 + 8 + cubNd];
            T temp1[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int ic1 = 0; ic1 < Nd; ++ic1)
                    for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq0 = 0; iq0 < Nq; ++iq0)
                            w1_d[Nq * Nq * iq0 + Nq * iq1 + iq2 + cubNq] += FE_TF1[iq2][ic2] * temp1[Nd * Nq * iq1 + Nd * iq0 + ic2];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < Nd; ++ic0)
                    for (int ic1 = 0; ic1 < Nd; ++ic1)
                        for (int ic2 = 0; ic2 < Nd; ++ic2)
                            temp0[Nd * Nd * iq0 + Nd * ic1 + ic2] += FE_TF1[iq0][ic0] * w[Nd * Nd * ic0 + Nd * ic1 + ic2 + 8 + cubNd];
            T temp1[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int ic1 = 0; ic1 < Nd; ++ic1)
                    for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq0 = 0; iq0 < Nq; ++iq0)
                            w1_d[Nq * Nq * iq0 + Nq * iq1 + iq2 + 4 * cubNq] += FE_TF1[iq2][ic2] * temp1[Nd * Nq * iq1 + Nd * iq0 + ic2];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq0 = 0; iq0 < Nq; ++iq0)
                            w1_d[Nq * Nq * iq0 + Nq * iq1 + iq2 + 7 * cubNq] += FE_TF0[iq2][ic2] * temp1[Nd * Nq * iq1 + Nd * iq0 + ic2];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < Nd; ++ic0)
                    for (int ic1 = 0; ic1 < Nd; ++ic1)
                        for (int ic2 = 0; ic2 < Nd; ++ic2)
                            temp0[Nd * Nd * iq0 + Nd * ic1 + ic2] += FE_TF0[iq0][ic0] * w[Nd * Nd * ic0 + Nd * ic1 + ic2 + 8 + 2 * cubNd];
            T temp1[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int ic1 = 0; ic1 < Nd; ++ic1)
                    for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq0 = 0; iq0 < Nq; ++iq0)
                            w1_d[Nq * Nq * iq0 + Nq * iq1 + iq2 + 2 * cubNq] += FE_TF1[iq2][ic2] * temp1[Nd * Nq * iq1 + Nd * iq0 + ic2];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int ic0 = 0; ic0 < Nd; ++ic0)
                    for (int ic1 = 0; ic1 < Nd; ++ic1)
                        for (int ic2 = 0; ic2 < Nd; ++ic2)
                            temp0[Nd * Nd * iq0 + Nd * ic1 + ic2] += FE_TF1[iq0][ic0] * w[Nd * Nd * ic0 + Nd * ic1 + ic2 + 8 + 2 * cubNd];
            T temp1[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int ic1 = 0; ic1 < Nd; ++ic1)
                    for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq0 = 0; iq0 < Nq; ++iq0)
                            w1_d[Nq * Nq * iq0 + Nq * iq1 + iq2 + 5 * cubNq] += FE_TF1[iq2][ic2] * temp1[Nd * Nq * iq1 + Nd * iq0 + ic2];
        }
        {
            T temp0[Nd * Nd * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
            ...
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int ic2 = 0; ic2 < Nd; ++ic2)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq0 = 0; iq0 < Nq; ++iq0)
                            w1_d[Nq * Nq * iq0 + Nq * iq1 + iq2 + 8 * cubNq] += FE_TF0[iq2][ic2] * temp1[Nd * Nq * iq1 + Nd * iq0 + ic2];
        }
        {
            T temp0[4 * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
        ...
        for (int iq0 = 0; iq0 < Nq; ++iq0)
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int iq2 = 0; iq2 < Nq; ++iq2)
                    for (int j = 0; j < 9; j++)
                    {
                        fw[9 * Nq * Nq * iq0 + 9 * Nq * iq1 + 9 * iq2 + j] = w1_d[9 * Nq * Nq * iq0 + 9 * Nq * iq1 + 9 * iq2 + j];
                    }
...
#if OPTIMIZE_SUM_FACTORIZATION == 1
        {
            ...
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int id = 0; id < Nq * Nq; ++id)
                        temp0[Nq * Nq * i0 + id] += FE_TF0[iq0][i0] * fw[Nq * Nq * iq0 + id];
            T temp1[Nd * Nd * Nq] = {0};
            ...
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int id = 0; id < Nq * Nq; ++id)
                        temp2[Nq * Nq * i0 + id] += FE_TF1[iq0][i0] * fw[Nq * Nq * iq0 + id + cubNq];
            T temp3[Nd * Nd * Nq] = {0};
            T temp2transp[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq2 = 0; iq2 < Nq; ++iq2)
            ...
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int id = 0; id < Nq * Nq; ++id)
                        temp4[Nq * Nq * i0 + id] += FE_TF1[iq0][i0] * fw[Nq * Nq * iq0 + id + 2 * cubNq];
            T temp5[Nd * Nd * Nq] = {0};
            T temp4transp[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq2 = 0; iq2 < Nq; ++iq2)
            ...
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int id = 0; id < Nq * Nq; ++id)
                        temp6[Nq * Nq * i0 + id] += FE_TF0[iq0][i0] * fw[Nq * Nq * iq0 + id + 3 * cubNq];
            T temp7[Nd * Nd * Nq] = {0};
            T temp6transp[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq2 = 0; iq2 < Nq; ++iq2)
                        temp6transp[Nd * Nq * iq1 + Nq * i0 + iq2] += temp6[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int id = 0; id < Nd * Nq; ++id)
                        temp7[Nd * Nq * i1 + id] += FE_TF1[iq1][i1] * temp6transp[Nd * Nq * iq1 + id];
            T temp7transp[Nd * Nd * Nq] = {0};
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        temp7transp[Nd * Nd * iq2 + Nd * i1 + i0] += temp7[Nd * Nq * i1 + Nq * i0 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int id = 0; id < Nd * Nd; ++id)
                        A[Nd * Nd * i2 + id] += FE_TF1[iq2][i2] * temp7transp[Nd * Nd * iq2 + id];
            T temp8[Nd * Nq * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int id = 0; id < Nq * Nq; ++id)
                        temp8[Nq * Nq * i0 + id] += FE_TF1[iq0][i0] * fw[Nq * Nq * iq0 + id + 4 * cubNq];
            T temp9[Nd * Nd * Nq] = {0};
            T temp8transp[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq2 = 0; iq2 < Nq; ++iq2)
                        temp8transp[Nd * Nq * iq1 + Nq * i0 + iq2] += temp8[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int id = 0; id < Nd * Nq; ++id)
                        temp9[Nd * Nq * i1 + id] += FE_TF0[iq1][i1] * temp8transp[Nd * Nq * iq1 + id];
            T temp9transp[Nd * Nd * Nq] = {0};
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        temp9transp[Nd * Nd * iq2 + Nd * i1 + i0] += temp9[Nd * Nq * i1 + Nq * i0 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int id = 0; id < Nd * Nd; ++id)
                        A[Nd * Nd * i2 + id] += FE_TF1[iq2][i2] * temp9transp[Nd * Nd * iq2 + id];
            T temp10[Nd * Nq * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int id = 0; id < Nq * Nq; ++id)
                        temp10[Nq * Nq * i0 + id] += FE_TF1[iq0][i0] * fw[Nq * Nq * iq0 + id + 5 * cubNq];
            T temp11[Nd * Nd * Nq] = {0};
            T temp10transp[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq2 = 0; iq2 < Nq; ++iq2)
                        temp10transp[Nd * Nq * iq1 + Nq * i0 + iq2] += temp10[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int id = 0; id < Nd * Nq; ++id)
                        temp11[Nd * Nq * i1 + id] += FE_TF1[iq1][i1] * temp10transp[Nd * Nq * iq1 + id];
            T temp11transp[Nd * Nd * Nq] = {0};
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        temp11transp[Nd * Nd * iq2 + Nd * i1 + i0] += temp11[Nd * Nq * i1 + Nq * i0 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int id = 0; id < Nd * Nd; ++id)
                        A[Nd * Nd * i2 + id] += FE_TF0[iq2][i2] * temp11transp[Nd * Nd * iq2 + id];
            T temp12[Nd * Nq * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int id = 0; id < Nq * Nq; ++id)
                        temp12[Nq * Nq * i0 + id] += FE_TF0[iq0][i0] * fw[Nq * Nq * iq0 + id + 6 * cubNq];
            T temp13[Nd * Nd * Nq] = {0};
            T temp12transp[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq2 = 0; iq2 < Nq; ++iq2)
                        temp12transp[Nd * Nq * iq1 + Nq * i0 + iq2] += temp12[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int id = 0; id < Nd * Nq; ++id)
                        temp13[Nd * Nq * i1 + id] += FE_TF1[iq1][i1] * temp12transp[Nd * Nq * iq1 + id];
            T temp13transp[Nd * Nd * Nq] = {0};
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        temp13transp[Nd * Nd * iq2 + Nd * i1 + i0] += temp13[Nd * Nq * i1 + Nq * i0 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int id = 0; id < Nd * Nd; ++id)
                        A[Nd * Nd * i2 + id] += FE_TF1[iq2][i2] * temp13transp[Nd * Nd * iq2 + id];
            T temp14[Nd * Nq * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int id = 0; id < Nq * Nq; ++id)
                        temp14[Nq * Nq * i0 + id] += FE_TF1[iq0][i0] * fw[Nq * Nq * iq0 + id + 7 * cubNq];
            T temp15[Nd * Nd * Nq] = {0};
            T temp14transp[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq2 = 0; iq2 < Nq; ++iq2)
                        temp14transp[Nd * Nq * iq1 + Nq * i0 + iq2] += temp14[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int id = 0; id < Nd * Nq; ++id)
                        temp15[Nd * Nq * i1 + id] += FE_TF0[iq1][i1] * temp14transp[Nd * Nq * iq1 + id];
            T temp15transp[Nd * Nd * Nq] = {0};
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        temp15transp[Nd * Nd * iq2 + Nd * i1 + i0] += temp15[Nd * Nq * i1 + Nq * i0 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int id = 0; id < Nd * Nd; ++id)
                        A[Nd * Nd * i2 + id] += FE_TF1[iq2][i2] * temp15transp[Nd * Nd * iq2 + id];
            T temp16[Nd * Nq * Nq] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int id = 0; id < Nq * Nq; ++id)
                        temp16[Nq * Nq * i0 + id] += FE_TF1[iq0][i0] * fw[Nq * Nq * iq0 + id + 8 * cubNq];
            T temp17[Nd * Nd * Nq] = {0};
            T temp16transp[Nd * Nq * Nq] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq2 = 0; iq2 < Nq; ++iq2)
                        temp16transp[Nd * Nq * iq1 + Nq * i0 + iq2] += temp16[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int id = 0; id < Nd * Nq; ++id)
                        temp17[Nd * Nq * i1 + id] += FE_TF1[iq1][i1] * temp16transp[Nd * Nq * iq1 + id];
            T temp17transp[Nd * Nd * Nq] = {0};
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        temp17transp[Nd * Nd * iq2 + Nd * i1 + i0] += temp17[Nd * Nq * i1 + Nq * i0 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int id = 0; id < Nd * Nd; ++id)
                        A[Nd * Nd * i2 + id] += FE_TF0[iq2][i2] * temp17transp[Nd * Nd * iq2 + id];
        }
#else
        {
            T temp0[Nq * Nq * Nd] = {0};
            ...
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp1[Nq * Nd * i1 + Nq * i0 + iq2] += FE_TF1[iq1][i1] * temp0[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int i1 = 0; i1 < Nd; ++i1)
                        for (int i0 = 0; i0 < Nd; ++i0)
                            A[Nd * Nd * i0 + Nd * i1 + i2 + 2 * cubNd] += FE_TF1[iq2][i2] * temp1[Nq * Nd * i1 + Nq * i0 + iq2];
            T temp2[Nq * Nq * Nd] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp2[Nq * Nq * i0 + Nq * iq1 + iq2] += FE_TF1[iq0][i0] * fw[Nq * Nq * iq0 + Nq * iq1 + iq2 + cubNq];
            T temp3[Nq * Nd * Nd] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp3[Nq * Nd * i1 + Nq * i0 + iq2] += FE_TF0[iq1][i1] * temp2[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int i1 = 0; i1 < Nd; ++i1)
                        for (int i0 = 0; i0 < Nd; ++i0)
                            A[Nd * Nd * i0 + Nd * i1 + i2 + 2 * cubNd] += FE_TF1[iq2][i2] * temp3[Nq * Nd * i1 + Nq * i0 + iq2];
            T temp4[Nq * Nq * Nd] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp4[Nq * Nq * i0 + Nq * iq1 + iq2] += FE_TF1[iq0][i0] * fw[Nq * Nq * iq0 + Nq * iq1 + iq2 + 2 * cubNq];
            T temp5[Nq * Nd * Nd] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp5[Nq * Nd * i1 + Nq * i0 + iq2] += FE_TF1[iq1][i1] * temp4[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int i1 = 0; i1 < Nd; ++i1)
                        for (int i0 = 0; i0 < Nd; ++i0)
                            A[Nd * Nd * i0 + Nd * i1 + i2 + 2 * cubNd] += FE_TF0[iq2][i2] * temp5[Nq * Nd * i1 + Nq * i0 + iq2];
            T temp6[Nq * Nq * Nd] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp6[Nq * Nq * i0 + Nq * iq1 + iq2] += FE_TF0[iq0][i0] * fw[Nq * Nq * iq0 + Nq * iq1 + iq2 + 3 * cubNq];
            T temp7[Nq * Nd * Nd] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp7[Nq * Nd * i1 + Nq * i0 + iq2] += FE_TF1[iq1][i1] * temp6[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int i1 = 0; i1 < Nd; ++i1)
                        for (int i0 = 0; i0 < Nd; ++i0)
                            A[Nd * Nd * i0 + Nd * i1 + i2 + 2 * cubNd] += FE_TF1[iq2][i2] * temp7[Nq * Nd * i1 + Nq * i0 + iq2];
            T temp8[Nq * Nq * Nd] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp8[Nq * Nq * i0 + Nq * iq1 + iq2] += FE_TF1[iq0][i0] * fw[Nq * Nq * iq0 + Nq * iq1 + iq2 + 4 * cubNq];
            T temp9[Nq * Nd * Nd] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp9[Nq * Nd * i1 + Nq * i0 + iq2] += FE_TF0[iq1][i1] * temp8[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int i1 = 0; i1 < Nd; ++i1)
                        for (int i0 = 0; i0 < Nd; ++i0)
                            A[Nd * Nd * i0 + Nd * i1 + i2 + 2 * cubNd] += FE_TF1[iq2][i2] * temp9[Nq * Nd * i1 + Nq * i0 + iq2];
            T temp10[Nq * Nq * Nd] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp10[Nq * Nq * i0 + Nq * iq1 + iq2] += FE_TF1[iq0][i0] * fw[Nq * Nq * iq0 + Nq * iq1 + iq2 + 5 * cubNq];
            T temp11[Nq * Nd * Nd] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp11[Nq * Nd * i1 + Nq * i0 + iq2] += FE_TF1[iq1][i1] * temp10[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int i1 = 0; i1 < Nd; ++i1)
                        for (int i0 = 0; i0 < Nd; ++i0)
                            A[Nd * Nd * i0 + Nd * i1 + i2 + 2 * cubNd] += FE_TF0[iq2][i2] * temp11[Nq * Nd * i1 + Nq * i0 + iq2];
            T temp12[Nq * Nq * Nd] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp12[Nq * Nq * i0 + Nq * iq1 + iq2] += FE_TF0[iq0][i0] * fw[Nq * Nq * iq0 + Nq * iq1 + iq2 + 6 * cubNq];
            T temp13[Nq * Nd * Nd] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp13[Nq * Nd * i1 + Nq * i0 + iq2] += FE_TF1[iq1][i1] * temp12[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int i1 = 0; i1 < Nd; ++i1)
                        for (int i0 = 0; i0 < Nd; ++i0)
                            A[Nd * Nd * i0 + Nd * i1 + i2 + 2 * cubNd] += FE_TF1[iq2][i2] * temp13[Nq * Nd * i1 + Nq * i0 + iq2];
            T temp14[Nq * Nq * Nd] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp14[Nq * Nq * i0 + Nq * iq1 + iq2] += FE_TF1[iq0][i0] * fw[Nq * Nq * iq0 + Nq * iq1 + iq2 + 7 * cubNq];
            T temp15[Nq * Nd * Nd] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp15[Nq * Nd * i1 + Nq * i0 + iq2] += FE_TF0[iq1][i1] * temp14[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int i1 = 0; i1 < Nd; ++i1)
                        for (int i0 = 0; i0 < Nd; ++i0)
                            A[Nd * Nd * i0 + Nd * i1 + i2 + 2 * cubNd] += FE_TF1[iq2][i2] * temp15[Nq * Nd * i1 + Nq * i0 + iq2];
            T temp16[Nq * Nq * Nd] = {0};
            for (int iq0 = 0; iq0 < Nq; ++iq0)
                for (int i0 = 0; i0 < Nd; ++i0)
                    for (int iq1 = 0; iq1 < Nq; ++iq1)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp16[Nq * Nq * i0 + Nq * iq1 + iq2] += FE_TF1[iq0][i0] * fw[Nq * Nq * iq0 + Nq * iq1 + iq2 + 8 * cubNq];
            T temp17[Nq * Nd * Nd] = {0};
            for (int iq1 = 0; iq1 < Nq; ++iq1)
                for (int i1 = 0; i1 < Nd; ++i1)
                    for (int i0 = 0; i0 < Nd; ++i0)
                        for (int iq2 = 0; iq2 < Nq; ++iq2)
                            temp17[Nq * Nd * i1 + Nq * i0 + iq2] += FE_TF1[iq1][i1] * temp16[Nq * Nq * i0 + Nq * iq1 + iq2];
            for (int iq2 = 0; iq2 < Nq; ++iq2)
                for (int i2 = 0; i2 < Nd; ++i2)
                    for (int i1 = 0; i1 < Nd; ++i1)
                        for (int i0 = 0; i0 < Nd; ++i0)
                            A[Nd * Nd * i0 + Nd * i1 + i2 + 2 * cubNd] += FE_TF0[iq2][i2] * temp17[Nq * Nd * i1 + Nq * i0 + iq2];
        }
#endif
    }
#if DEGREE == 1
